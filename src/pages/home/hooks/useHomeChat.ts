...
// 提取引用信息（参考探索页面）
const extractReferencesFromSSEData = (data: any): any[] => {
  if (!data?.reference?.chunks) return []
...
export const useHomeChat = ({ 
  ...
  const loadConversationHistory = useCallback(async (conversationId: string): Promise<ChatMessage[]> => {
    ...
    try {
      ...
      if (response?.message && Array.isArray(response.message)) {
        const loadedMessages: ChatMessage[] = response.message.map((msg: any) => ({
  ...
  useEffect(() => {
    ...
    } else if (selectedConversationId === null && currentConversationIdRef.current !== null) {
      // 新建对话时清空消息
      updateMessages([])
      updateCurrentConversationId(null)
    }
  }, [selectedConversationId, loadConversationHistory, updateMessages, updateCurrentConversationId])
  ...
  // 这样就不会出现重复渲染的问题
  const sendAppMessage = useCallback(async (inputValue: string) => {
    ...
    try {
      ...
      let conversationId = selectedConversationId || currentConversationIdRef.current
      ...
      if (selectedConversationId && selectedConversationId !== currentConversationIdRef.current) {
        console.log('[sendAppMessage] 检测到历史对话未加载，同步加载中...')
        // 同步加载历史消息，确保 currentMessages 是正确的历史
        currentMessages = await loadConversationHistory(selectedConversationId)
        ...
        const newConversation = await conversationAPI.setConversation({
          ...
          name: userMessageContent.slice(0, 50) + (userMessageContent.length > 50 ? '...' : ''),
          is_new: true
        })
      ...
      const requestMessages = [...currentMessages, userMessage].map(msg => ({
        role: msg.role,
        content: msg.content
      }))
  ...
  }, [selectedApp, selectedConversationId, onConversationIdChange, loadConversationHistory, isLoadingHistory, updateMessages, updateCurrentConversationId])

  // 发送消息 - MCP 模式
  const sendMCPMessage = useCallback(async (inputValue: string) => {
