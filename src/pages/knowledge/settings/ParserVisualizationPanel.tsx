...
const ParserVisualizationPanel: React.FC<ParserVisualizationPanelProps> = ({
  ...
  const detailedInfo = getParserDetailedDescription(
    selectedParser as DocumentParserType,
  )
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {parserInfo.images.map((imageName, index) => (
              <Card
                key={index}
                className="border-border/50 bg-background-subtle p-3 transition-colors hover:border-border"
              >
...
// 获取解析器详细说明信息
function getParserDetailedDescription(parserType: DocumentParserType): {
  description: string
  supportedFormats: string
} {
  const descriptions: Record<
    DocumentParserType,
    { description: string; supportedFormats: string }
  > = {
...
在标签列中，标签之间使用英文逗号分隔。`,
  ...
  return (
    descriptions[parserType] || {
      description: '请配置相应的解析参数以获得最佳效果。',
      supportedFormats: '多种格式',
    }
  )
}
