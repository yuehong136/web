...
const toggleRowClass =
  'flex items-center justify-between gap-space-base py-space-xs'
    ...
    >
      ...
      >
        ...
        <SliderWithInput
          ...
          value={
            config.llm_setting.temperature ?? SLIDER_PRESETS.temperature.default
          }
          onChange={(value) => handleLLMSettingChange('temperature', value)}
          enabled={config.llm_setting.temperature_enabled ?? false}
          onEnabledChange={(checked) =>
            handleLLMSettingChange('temperature_enabled', checked)
          }
        ...
        <SliderWithInput
          ...
          onEnabledChange={(checked) =>
            handleLLMSettingChange('top_p_enabled', checked)
          }
        ...
        <SliderWithInput
          ...
          value={
            config.llm_setting.presence_penalty ??
            SLIDER_PRESETS.presencePenalty.default
          }
          onChange={(value) =>
            handleLLMSettingChange('presence_penalty', value)
          }
          enabled={config.llm_setting.presence_penalty_enabled ?? false}
          onEnabledChange={(checked) =>
            handleLLMSettingChange('presence_penalty_enabled', checked)
          }
        ...
        <SliderWithInput
          ...
          value={
            config.llm_setting.frequency_penalty ??
            SLIDER_PRESETS.frequencyPenalty.default
          }
          onChange={(value) =>
            handleLLMSettingChange('frequency_penalty', value)
          }
          enabled={config.llm_setting.frequency_penalty_enabled ?? false}
          onEnabledChange={(checked) =>
            handleLLMSettingChange('frequency_penalty_enabled', checked)
          }
        ...
        <SliderWithInput
          ...
          value={
            config.llm_setting.max_tokens ?? SLIDER_PRESETS.maxTokens.default
          }
          onChange={(value) => handleLLMSettingChange('max_tokens', value)}
          enabled={config.llm_setting.max_tokens_enabled ?? false}
          onEnabledChange={(checked) =>
            handleLLMSettingChange('max_tokens_enabled', checked)
          }
        ...
        <div className="space-y-space-xl rounded-radius-lg bg-surface-secondary/40 px-space-lg py-space-lg">
          ...
          <div className="space-y-space-xl py-space-sm">
            ...
            <div className="space-y-space-base py-space-sm">
              ...
              <Slider
                ...
                onValueChange={(values) =>
                  handleConfigChange('similarity_threshold', values[0])
                }
              />
            </div>
            ...
            <div className="space-y-space-base py-space-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="gap-space-xs flex items-center text-xs">
                  ...
                  <span className="text-text-secondary">
                    full-text:{' '}
                    {(1 - Number(config.vector_similarity_weight ?? 0)).toFixed(
                      2,
                    )}
                  </span>
                </div>
              ...
              <Slider
                ...
                onValueChange={(values) =>
                  handleConfigChange('vector_similarity_weight', values[0])
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-space-xl py-space-sm">
            ...
            <SliderWithInput
              ...
              value={config.top_n}
              onChange={(value) =>
                handleConfigChange('top_n', Math.max(1, Math.round(value)))
              }
              min={1}
              max={30}
              step={1}
            ...
            <SliderWithInput
              ...
              value={config.top_k}
              onChange={(value) =>
                handleConfigChange('top_k', Math.max(1, Math.round(value)))
              }
              min={1}
              max={2048}
              step={1}
                ...
                <button
                  ...
                  onClick={() => {
                    const nextLanguages = selected
                      ...
                      : [
                          ...config.prompt_config.cross_languages,
                          language.value,
                        ]
      ...
      >
        {config.prompt_config.parameters.length === 0 ? (
          <div className="py-space-base text-center text-sm text-text-tertiary">
            暂无变量
          </div>
        ) : (
          <div className="space-y-space-sm">
