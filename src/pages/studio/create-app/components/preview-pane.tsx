...
import { Bug, ChevronLeft, ChevronRight, Eye, LayoutGrid, RefreshCw, Square } from 'lucide-react'
...
export const PreviewPane: React.FC<PreviewPaneProps> = ({ controller }) => {
      ...
      return {
        ...
        avatar: isUser ? (
          ...
          <div className="h-8 min-h-[32px] w-8 min-w-[32px] shrink-0 overflow-hidden rounded-radius-full">
            <img src={config.icon} alt="AI" className="h-full w-full object-cover" />
          </div>
        ) : (
          <div
  ...
  return (
    ...
    >
      <ScrollArea className="min-h-0 flex-1" viewportClassName="overscroll-contain p-space-base">
        {previewMessages.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <h3 className="mb-space-sm text-lg font-semibold text-text-primary">{config.name}</h3>
            <p className="mx-auto max-w-md text-sm text-text-secondary">{config.description}</p>
          </div>
        ) : (
      ...
      <div className="shrink-0 border-t border-border-default p-space-base">
        <div className="mb-space-base">
          ...
          {isStreaming ? (
            <div className="mt-space-sm flex justify-center">
              ...
              >
                <Square className="mr-space-xs h-4 w-4" style={{ color: 'var(--color-state-error-text)' }} />
                停止生成
              </Button>
            </div>
          ) : null}
        ...
        <div className="rounded-radius-lg p-space-base" style={{ backgroundColor: 'var(--color-chat-preview-debug-bg)' }}>
          <span className="mb-space-sm block text-xs font-medium" style={{ color: 'var(--color-chat-preview-debug-text)' }}>
            调试信息
          </span>
          <div className="space-y-1">
            ...
            <div className="flex justify-between text-xs">
              ...
              <span style={{ color: 'var(--color-chat-preview-debug-text)' }}>
                {Number(config.llm_setting.temperature ?? 0).toFixed(2) || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between text-xs">
