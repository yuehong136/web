...
interface DialogSearchModeHybrid {
  weight_dense?: number
...
interface DialogDetailResponse {
  ...
  kb_ids?: string[]
  search_mode?: Record<string, DialogSearchModeHybrid> | null
...
export const normalizeDialogConfig = (data: DialogDetailResponse) => {
  ...
  const config: AppConfig = {
    ...
    search_mode: (() => {
      ...
      if (keys.length > 0) {
        ...
        const modeData = data.search_mode[modeType]
  ...
  const matchedPreset = detectMatchingPresetSnake({
    ...
    presence_penalty_enabled: config.llm_setting.presence_penalty_enabled ?? false,
    frequency_penalty_enabled: config.llm_setting.frequency_penalty_enabled ?? false,
...
export const mapLLMProviders = (response: Record<string, ProviderResponseEntry>) => {
