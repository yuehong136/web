  ...
  if (!Array.isArray(trace) || trace.length === 0) {
    return (
      <div className="py-space-xl text-center text-sm text-text-secondary">
        Pipeline 暂未启动或还没有日志，可在 Run 视图填写 Begin 输入并启动。
      </div>
    )
  }
  ...
  return (
    <div className="space-y-space-base">
      {trace.map((item, idx) => {
        ...
        return (
          <div key={`${item.component_id || idx}-${idx}`} className="relative pl-space-xl">
            {idx < trace.length - 1 ? (
              <div className="absolute bottom-0 left-[11px] top-8 w-0.5 bg-surface-accent" />
            ) : null}
            ...
            <div className="absolute left-0 top-0 flex items-center justify-center">
              ...
              >
                <OperatorIcon
                  className="size-4"
                  name={(item.component_name as string) || item.component_id || ''}
                />
              </div>
            </div>
                ...
                >
                  ...
                  <AccordionContent>
                    <div className="space-y-space-sm">
                      {typeof progress === 'number' ? (
                        <div>
                          <div className="mb-space-xs text-xs text-text-secondary">进度</div>
                          <div className="h-1.5 overflow-hidden rounded-radius-full bg-surface-secondary">
                            <div
                              className="h-full rounded-radius-full bg-surface-accent"
                      ...
                      <div>
                        ...
                        <pre className="max-h-40 overflow-auto rounded-radius-sm bg-surface-secondary p-space-sm text-xs">
                      ...
                      {typeof item.message === 'string' && item.message ? (
                        <div>
                          <div className="mb-space-xs text-xs text-text-secondary">消息</div>
                          <pre className="max-h-40 overflow-auto rounded-radius-sm bg-surface-secondary p-space-sm text-xs">
                            {item.message}
                          </pre>
                        </div>
                      ...
                      {traceMessages.length > 0 ? (
                        <div>
                          <div className="mb-space-xs text-xs text-text-secondary">运行消息</div>
