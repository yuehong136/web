...
import { z } from 'zod'
import { initialKeywordExtractValues } from '../constant'
...
export function KeywordExtractForm({ node }: INextOperatorForm) {
  const { t } = useTranslation()
  const values = useFormValues(initialKeywordExtractValues, node)
    ...
    <Form {...form}>
      <FormWrapper>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('flow.topN', 'Top N Keywords')}</FormLabel>
              <FormControl>
                <Input
                  type="number"
                  min={1}
