...
const MODEL_TYPES = {
  ...
  embd_id: {
    label: 'Embedding',
    types: ['embedding'],
    tooltip: '所有新创建的知识库使用的默认嵌入模型。如未显示可选模型，请检查你是否在使用 MultiRAG slim 版(不含嵌入模型)；或确认你的模型供应商是否提供该模型。'
  },
  img2txt_id: {
    label: 'VLM',
    types: ['image2text'],
    tooltip: '所有新创建的知识库都将使用默认的 VLM 模型。它可以描述图片或视频。如未显示可选模型，请确认你的模型供应商是否提供该模型。'
  },
  asr_id: {
    label: 'ASR',
    types: ['speech2text'],
    tooltip: '所有新创建的知识库都将使用默认的 ASR 模型。使用此模型将语音翻译为相应的文本。如未显示可选模型，请确认你的模型供应商是否提供该模型。'
  },
  rerank_id: {
    label: 'Rerank',
    types: ['rerank'],
    tooltip: '默认的 reranking 模型。如未显示可选模型，请确认你的模型供应商是否提供该模型。'
  },
  tts_id: {
    label: 'TTS',
    types: ['tts'],
    tooltip: '默认的 TTS 模型会被用于在对话过程中请求语音生成时使用。如未显示可选模型，请确认你的模型供应商是否提供该模型。'
  }
} as const
...
export const SystemSetting: React.FC = () => {
  ...
  const getGroupedOptionsForType = useCallback((types: string[]): SelectOptionGroup[] => {
      ...
      if (matchingModels.length > 0) {
        groups.push({
          ...
          options: matchingModels.map(model => ({
            // 选项：模型名称（带图标）
            label: <ModelOptionLabel provider={providerName} modelName={model.name} />,
  ...
  // 处理字段变更并保存（传递完整的设置对象）
  const handleFieldChange = useCallback(async (field: keyof TenantInfo, value: string) => {
  ...
  return (
    <div className="rounded-lg w-full">
      ...
      {/* 设置表单 */}
      <div className="px-7 py-6 space-y-6 border border-border-subtle rounded-lg max-h-[70vh] overflow-y-auto">
        {(Object.keys(MODEL_TYPES) as Array<keyof typeof MODEL_TYPES>).map((field) => {
