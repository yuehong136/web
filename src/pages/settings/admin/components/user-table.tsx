...
const StatusPill: React.FC<{ active: boolean | null }> = memo(({ active }) => (
  <span
    className={cn(
      ...
      active
        ? 'bg-status-success-subtle text-text-success'
...
export const UserTable: React.FC<UserTableProps> = memo(
  ({
    ...
    const handleChangePassword = useCallback(
      (user: AdminUser) => onChangePassword(user),
      [onChangePassword],
    )
    const handleToggleStatus = useCallback(
      (user: AdminUser) => onToggleStatus(user),
      [onToggleStatus],
    )
    const handleGrantAdmin = useCallback(
      (user: AdminUser) => onGrantAdmin(user),
      [onGrantAdmin],
    )
    const handleDelete = useCallback(
      (user: AdminUser) => onDelete(user),
      [onDelete],
    )
