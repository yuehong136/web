export default {
  flow: {
    switchPrompt: 'Switch prompt preset',
    model: 'Model',
    modelSettings: 'Model settings',
    selectModel: 'Select model',
    noModels: 'No models available',
    temperature: 'Temperature',
    topP: 'Top P',
    presencePenalty: 'Presence penalty',
    frequencyPenalty: 'Frequency penalty',
    maxTokens: 'Max tokens',
    parser: 'Parser',
    parserDescription:
      'Extract raw text and structure from incoming files before downstream processing.',
    fileFormat: 'File format',
    fileFormatOptions: {
      pdf: 'PDF',
      spreadsheet: 'Spreadsheet',
      image: 'Image',
      email: 'Email',
      textMarkdown: 'Text / Markdown',
      word: 'Word',
      slides: 'Slides',
      audio: 'Audio',
      video: 'Video',
    },
    preprocess: {
      preprocess: 'Preprocess',
      mainContent: 'Main content',
      sectionTitle: 'Section title',
      abstract: 'Abstract',
      author: 'Author',
    },
    outputFormat: 'Output format',
    fileFormats: 'File formats',
    fields: 'Fields',
    questions: 'Questions',
    summary: 'Summary',
    searchMethod: 'Search method',
    searchMethodTip:
      'Choose whether the tokenizer uses embedding search, full-text search, or both.',
    filenameEmbeddingWeight: 'Filename embedding weight',
    tokenizerSearchMethodOptions: {
      embedding: 'Embedding',
      full_text: 'Full text',
    },
    tokenizerFieldsOptions: {
      text: 'Text',
      questions: 'Questions',
      summary: 'Summary',
    },
    addParser: 'Add parser',
    parserAddDisabled: 'All parser types already added',
    parserList: 'Parsers',
    parserStates: {
      edited: 'Edited',
      needsAttention: 'Needs attention',
    },
    parserMethod: 'Parse method',
    tableResultType: 'Table result type',
    markdownImageResponseType: 'Markdown image response type',
    systemPrompt: 'System prompt',
    systemPromptPlaceholder:
      'Enter a system prompt for image analysis. Leave empty to use the default prompt.',
    promptWorkspace: 'Prompt workspace',
    language: 'Language',
    lang: 'Language',
    mineruParseMethod: 'MinerU parse method',
    mineruLanguage: 'MinerU language',
    mineruFormulaEnable: 'Formula recognition',
    mineruTableEnable: 'Table recognition',
    paddleocrParseMethod: 'PaddleOCR parse method',
    doclingParseMethod: 'Docling parse method',
    noParsersConfigured: 'No parsers configured',
    noInputs: 'No inputs configured',
    insertVariable: 'Insert variable',
    insertVariableTip: 'Type / to browse upstream variables',
    tools: 'Tools',
    agent: 'Agent',
    addAgent: 'Add Agent',
    addTools: 'Add Tools',
    noToolsSelected: 'No tools selected yet.',
    noAgentsSelected: 'No sub-agents yet.',
    builtIn: 'Built-in',
    search: 'Search',
    communication: 'Communication',
    developer: 'Developer',
    typeCommandORsearch: 'Type a command or search...',
    noResultsFound: 'No results found.',
    toolNoConfig: "It doesn't have any config.",
    switchConditionValuePlaceholder: 'Enter the value to compare with...',
    switchOperatorOptions: {
      equal: 'Equal',
      notEqual: 'Not equal',
      gt: 'Greater than',
      ge: 'Greater than or equal',
      lt: 'Less than',
      le: 'Less than or equal',
      contains: 'Contains',
      notContains: 'Does not contain',
      startWith: 'Starts with',
      endWith: 'Ends with',
      empty: 'Empty',
      notEmpty: 'Not empty',
      in: 'In',
      notIn: 'Not in',
      is: 'Is',
      isNot: 'Is not',
    },
    switchLogicOperatorOptions: {
      and: 'And',
      or: 'Or',
    },
    userPromptDefaultValue: 'This is the order you need to send to the agent.',
    descriptionMessage: 'This is an agent for a specific task.',
    sysPromptDefaultValue: 'You are a helpful AI assistant.',
    content: 'Content',
    title: 'Title',
    subtitle: 'Subtitle',
    examples: 'Examples',
    addCategory: 'Add Category',
    categoryName: 'Category name',
    nameRequiredMsg: 'Name is required',
    nameRepeatedMsg: 'Name already exists',
    logoImage: 'Logo image',
    logoPosition: 'Logo position',
    logoWidth: 'Logo width',
    logoHeight: 'Logo height',
    fontFamily: 'Font family',
    fontSize: 'Font size',
    titleFontSize: 'Title font size',
    pageSize: 'Page size',
    orientation: 'Orientation',
    marginTop: 'Margin top',
    marginBottom: 'Margin bottom',
    filename: 'Filename',
    outputDirectory: 'Output directory',
    addPageNumbers: 'Add page numbers',
    addTimestamp: 'Add timestamp',
    watermarkText: 'Watermark text',
    selectVariable: 'Select variable',
    variablePickerEmptyTitle: 'No variables found',
    selectKnowledgeBases: 'Select knowledge bases',
    "ExceptionDefaultValue": "Exception default value",
    "add": "Add",
    "addExpression": "Add Expression",
    "addField": "Add option",
    "addGroup": "Add Group",
    "addMessage": "Add message",
    "addVariable": "Add variable",
    "advanced": "advanced",
    "advancedSettings": "Advanced settings",
    "apiKey": "API KEY",
    "authCode": "SMTP password / app password",
    "autoPlay": "Auto play audio",
    "balanceSheet": "Balance sheet",
    "basic": "basic",
    "begin": "Begin",
    "beginInput": "Begin input",
    "boolean": "Boolean",
    "cashFlowStatement": "Cash flow statement",
    "ccEmail": "CC email",
    "channel": "Channel",
    "childrenDelimiters": "Children Delimiters",
    "chunkTokenSize": "Chunk Token Size",
    "chunker": "Chunker",
    "cite": "Cite",
    "cleanHtml": "Clean HTML",
    "code": "Code",
    "componentId": "Component ID",
    "conversationVariable": "Conversation variable",
    "conversational": "Conversational",
    "copy": "Copy",
    "country": "Country & region",
    "crossLanguages": "Cross Languages",
    "dataManipulation": "Data manipulation",
    "database": "Database",
    "datatype": "Datatype",
    "days": "Days",
    "dbType": "Database type",
    "debug": "Debug",
    "debugComingSoon": "Single-node debugging is under development",
    "delayAfterError": "Delay after error",
    "delete": "Delete",
    "delimiters": "Delimiters",
    "description": "Description",
    "dialog": "Dialogue",
    "downloadFileType": "Download file type",
    "editVariable": "Edit variable",
    "email": "Email",
    "embeddingModel": "Embedding Model",
    "emptyResponse": "Empty Response",
    "enableChildrenDelimiters": "Child chunk are used for retrieval",
    "error": "Error",
    "exceptionMethod": "Exception method",
    "excludeDomains": "Exclude Domains",
    "extractDepth": "Extract depth",
    "extractType": "Extract type",
    "field": "Field",
    "fieldName": "Result destination",
    "file": "File upload",
    "financials": "Financials",
    "flow": "Flow",
    "flowNum": "N",
    "format": "Format",
    "foundation": "Foundation",
    "frameworkPrompts": "Framework",
    "groupName": "Group name",
    "guidingQuestion": "Guidance question",
    "headers": "Headers",
    "hierarchy": "Hierarchy",
    "history": "History",
    "host": "Host",
    "imageTableContextWindow": "Image & Table Context Window",
    "includeAnswer": "Include answer",
    "includeDomains": "Include domains",
    "includeImageDescriptions": "Include image descriptions",
    "includeImages": "Include images",
    "includeRawContent": "Include raw content",
    "info": "Info",
    "input": "Input",
    "integer": "Number",
    "iterationEmptyHint": "Drag nodes here, or add child nodes from the right panel",
    "iterationEmptySubHint": "The left Start anchor is the entry for each iteration",
    "iterationItems": "Find variable",
    "iterationOutputsEmpty": "No output items yet. Add child nodes first, then reference their variables here.",
    "key": "Key",
    "keyCannotRepeat": "The key cannot be repeated.",
    "keywordsSimilarityWeight": "Keywords Similarity Weight",
    "knowledgeBases": "Knowledge Bases",
    "languagePlaceholder": "Select language",
    "line": "Single-line text",
    "loading": "Loading...",
    "logic": "Logic",
    "loopTerminationCondition": "Loop termination condition",
    "loopVariables": "Loop variables",
    "manual": "Manual",
    "maxRecords": "Max records",
    "maxResults": "Max results",
    "maxRetries": "Max retry rounds",
    "maxRounds": "Max reflection rounds",
    "maximumLoopCount": "Maximum loop count",
    "mcpSelect": "Select MCP servers",
    "messageHistoryWindowSize": "Message window size",
    "messagePlaceholder": "Please enter your message content, use '/' to quickly insert variables.",
    "metadataFilter": "Metadata Filter",
    "metadataFilterDescription": "Filter dataset chunks by metadata conditions before retrieval runs.",
    "metadataFilterSelectDatasetFirst": "Select a dataset first to load metadata fields.",
    "method": "Method",
    "mode": "Mode",
    "modeTip": "The mode defines how the workflow is initiated.",
    "msg": "Messages",
    "name": "Name",
    "newKey": "New key",
    "news": "News",
    "nextStep": "Next step",
    "noKnowledgeBases": "No knowledge bases available",
    "noLogs": "No logs yet",
    "noMcpServers": "No MCP servers",
    "noParameters": "No parameters yet",
    "noSearchMethod": "No search methods configured",
    "noVariables": "No variables yet",
    "note": "Note",
    "num": "Num",
    "oldKey": "Old key",
    "onFailure": "On failure",
    "openingCopy": "Opening greeting",
    "openingSwitch": "Opening switch",
    "openingSwitchTip": "Your users will see this welcome message at the beginning.",
    "operationResults": "Operation results",
    "operations": "Operations",
    "operationsOptions":  {
      "appendOrUpdate": "Append or update",
      "filterValues": "Filter values",
      "removeKeys": "Remove keys",
      "renameKeys": "Rename keys",
      "selectKeys": "Select keys",
    },
    "operator": "Operator",
    "optional": "Optional",
    "options": "Dropdown options",
    "output": "Output",
    "outputName": "Output name",
    "overlappedPercent": "Overlapped percent (%)",
    "paragraph": "Paragraph text",
    "parameter": "Parameter",
    "password": "Password",
    "patents": "Patents",
    "port": "Port",
    "proxy": "Proxy",
    "query": "Query variables",
    "queryTip": "Select the variable you want to use",
    "queryType": "Query type",
    "regularExpression": "Regular Expression",
    "rerankModel": "Rerank Model",
    "retrievalFrom": "Retrieval from",
    "saveToMemory": "Save to memory",
    "script": "Script",
    "searchDepth": "Search depth",
    "searxngUrl": "SearXNG URL",
    "security": "Security",
    "selectCrossLanguages": "Select languages",
    "selectEmbeddingModel": "Select embedding model",
    "selectField": "Select field",
    "selectFields": "Select metadata fields",
    "selectMemories": "Select memories",
    "selectRerankModel": "Select rerank model",
    "semiAuto": "Semi Auto",
    "senderEmail": "From email address",
    "senderName": "From display name",
    "similarityThreshold": "Similarity Threshold",
    "smtpPort": "SMTP port",
    "smtpServer": "SMTP host",
    "sortBy": "Sort by",
    "sortMethod": "Sort method",
    "splitRef": "Split Ref",
    "sqlStatement": "SQL statement",
    "start": "Start",
    "stockCode": "Stock code",
    "structuredOutput":  {
      "configuration": "Configuration",
      "structuredOutput": "Structured output",
    },
    "subject": "Subject",
    "suggestedValues": "Suggested values",
    "switchPromptMessage": "The prompt words will change. Please confirm whether you want to discard the existing prompt words?",
    "task": "Task",
    "test": "Test",
    "timeout": "Timeout",
    "toEmail": "Recipient email",
    "tocDataSource": "TOC Data Source",
    "tocEnhance": "TOC Enhance",
    "topK": "Top K",
    "topN": "Top N Results",
    "topic": "Topic",
    "type": "Type",
    "urls": "URLs",
    "useKnowledgeGraph": "Use Knowledge Graph",
    "userId": "User ID",
    "userPrompt": "User prompt",
    "username": "Username",
    "value": "Value",
    "variable": "Variable",
    "variableSettings": "Variable settings",
    "variables": "Variables",
    "visualInputFile": "Visual Input File",
    "webhook":  {
      "algorithm": "Algorithm",
      "audience": "Audience",
      "authType": "Authentication type",
      "bodyTemplate": "Body template",
      "contentTypes": "Content types",
      "executionMode": "Execution mode",
      "executionModeTip": "Accepted Response: The system returns an acknowledgment immediately after the request is validated, while the workflow continues to execute asynchronously in the background. /Final Response: The system returns a response only after the workflow execution is completed.",
      "headerParameters": "Header parameters",
      "ipWhitelist": "IP whitelist",
      "issuer": "Issuer",
      "limit": "Request frequency limit",
      "maxBodySize": "Maximum body size",
      "methods": "Methods",
      "name": "Webhook",
      "password": "Password",
      "per": "Time period",
      "queryParameters": "Query parameters",
      "requestBodyParameters": "Request body parameters",
      "requiredClaims": "Required claims",
      "response": "Response",
      "schema": "Schema",
      "secret": "Secret",
      "status": "Status",
      "tokenHeader": "Token header",
      "tokenValue": "Token value",
      "username": "Username",
    },
    "yearHigh": "Year high",
    "yearLow": "Year low",
    "agentDescription": "Builds agent components equipped with reasoning, tool usage, and multi-agent collaboration. ",
    "arXiv": "ArXiv",
    "arXivDescription": "A component that searches from https://arxiv.org/, allowing you to specify the number of search results using TopN. It supplements the existing datasets.",
    "beginDescription": "This is where the flow begins.",
    "bing": "Bing",
    "bingDescription": "A component that searches from https://www.bing.com/, allowing you to specify the number of search results using TopN. It supplements the existing datasets. Please note that this requires an API key from microsoft.com.",
    "categorize": "Categorize",
    "categorizeDescription": "A component that uses the LLM to classify user inputs into predefined categories. Ensure you specify the name, description, and examples for each category, along with the corresponding next component.",
    "codeExec": "Code",
    "codeExecDescription": "Write your custom Python or Javascript logic.",
    "comma": "Comma",
    "comment": "Default value",
    "crawler": "Web crawler",
    "crawlerDescription": "A component that crawls HTML source code from a specified URL.",
    "dataOperations": "Data operations",
    "dataOperationsDescription": "Perform various operations on a Data object.",
    "date": "Date",
    "duckDuckGo": "DuckDuckGo",
    "duckDuckGoDescription": "A component that searches from duckduckgo.com, allowing you to specify the number of search results using TopN. It supplements the existing datasets.",
    "emailDescription": "Send an email to a specified address.",
    "excelProcessor": "Excel Processor",
    "excelProcessorDescription": "Excel Processor node",
    "exeSQL": "Execute SQL",
    "exeSQLDescription": "A component that performs SQL queries on a relational database, supporting querying from MySQL, PostgreSQL, or MariaDB.",
    "exitLoop": "Exit loop",
    "exitLoopDescription": "Equivalent to \"break\". This node has no configuration items. When the loop body reaches this node, the loop terminates.",
    "extractor": "Transformer",
    "extractorDescription": "Use an LLM to extract structured insights from document chunks—such as summaries, classifications, etc.",
    "fileDescription": "File node",
    "general": "general",
    "gitHub": "GitHub",
    "gitHubDescription": "A component that searches for repositories from https://github.com/. You can use Top N to specify the number of search results.",
    "google": "Google",
    "googleDescription": "A component that searches from https://www.google.com/, allowing you to specify the number of search results using TopN. It supplements the existing datasets. Please note that this requires an API key from serpapi.com.",
    "googleScholar": "Google Scholar",
    "googleScholarDescription": "A component that searches https://scholar.google.com/. You can use Top N to specify the number of search results.",
    "goto": "Exception branch",
    "hierarchicalMerger": "Title",
    "hierarchicalMergerDescription": "Split documents into sections by title hierarchy with regex rules for finer control.",
    "invoke": "HTTP request",
    "invokeDescription": "A component capable of calling remote services, using other components' outputs or constants as inputs.",
    "iteration": "Iteration",
    "iterationDescription": "A looping component that iterates over an input array and executes a defined logic for each item.",
    "iterationItem": "Iteration Item",
    "iterationItemDescription": "It represents the current element in the iteration, which can be referenced and manipulated in subsequent steps.",
    "keywordExtract": "Keyword",
    "keywordExtractDescription": "A component that extracts keywords from a user query, with Top N specifying the number of keywords to extract.",
    "lastUpdatedDate": "Last updated date",
    "lineBreak": "Line break",
    "listOperations": "List operations",
    "listOperationsDescription": "Perform operations on a list.",
    "loop": "Loop",
    "loopDescription": "Loop is the upper limit of the number of loops of the current component, when the number of loops exceeds the value of loop, it means that the component can not complete the current task, please re-optimize agent",
    "loopItem": "Loop Item",
    "loopItemDescription": "Loop Item node",
    "markdown": "Markdown",
    "merge": "Merge",
    "message": "Message",
    "messageDescription": "This component returns the final data output of the workflow along with predefined message content. ",
    "noteDescription": "Note",
    "pDFGenerator": "Docs Generator",
    "pDFGeneratorDescription": "A component that generates documents (PDF, DOCX, TXT) from markdown-formatted content with customizable styling, images, and tables. Supports: **bold**, *italic*, # headings, - lists, tables with | syntax.",
    "period": "Period",
    "placeholder": "Placeholder",
    "placeholderDescription": "Placeholder node",
    "pubMed": "PubMed",
    "pubMedDescription": "A component that searches from https://pubmed.ncbi.nlm.nih.gov/, allowing you to specify the number of search results using TopN. It supplements the existing datasets.",
    "relevance": "Relevance",
    "relevant": "Relevant",
    "relevantDescription": "A component that uses the LLM to assess whether the upstream output is relevant to the user's latest query. Ensure you specify the next component for each judge result.",
    "retrieval": "Retrieval",
    "retrievalDescription": "A component that retrieves information from specified datasets (datasets). Ensure that the datasets you select use the same embedding model.",
    "rewriteQuestion": "Rewrite",
    "rewriteQuestionDescription": "A component that rewrites a user query from the Interact component, based on the context of previous dialogues.",
    "searXNG": "SearXNG",
    "searXNGDescription": "A component that searches via your provided SearXNG instance URL. Specify TopN and the instance URL.",
    "semicolon": "Semicolon",
    "space": "Space",
    "split": "Split",
    "splitter": "Token",
    "splitterDescription": "Split text into chunks by token length with optional delimiters and overlap.",
    "stringTransform": "Text processing",
    "stringTransformDescription": "Modifies text content. Currently supports: Splitting or concatenating text.",
    "submittedDate": "Submitted date",
    "switch": "Switch",
    "switchDescription": "A component that evaluates conditions based on the output of previous components and directs the flow of execution accordingly. It allows for complex branching logic by defining cases and specifying actions for each case or default action if no conditions are met.",
    "tab": "Tab",
    "tavilyExtract": "Tavily extract",
    "tavilyExtractDescription": "Tavily Extract",
    "tavilySearch": "Tavily search",
    "tavilySearchDescription": "Search results via Tavily service.",
    "text": "Text",
    "tokenizer": "Indexer",
    "tokenizerDescription": "Transforms text into the required data structure (e.g., vector embeddings for Embedding Search) depending on the chosen search method.",
    "tool": "Tool",
    "toolDescription": "Tool node",
    "userFillUp": "Await response",
    "userFillUpDescription": "Pauses the workflow and waits for the user's message before continuing.",
    "variableAggregator": "Variable aggregator",
    "variableAggregatorDescription": "\nThis process aggregates variables from multiple branches into a single variable to achieve unified configuration for downstream nodes.",
    "variableAssigner": "Variable assigner",
    "variableAssignerDescription": "This component performs operations on Data objects, including extracting, filtering, and editing keys and values in the Data.",
    "waitingDialogue": "Waiting Dialogue",
    "waitingDialogueDescription": "Waiting Dialogue node",
    "wenCai": "WenCai",
    "wenCaiDescription": "A component that obtains financial information, including stock prices and funding news, from a wide range of financial websites.",
    "wikipedia": "Wikipedia",
    "wikipediaDescription": "A component that searches from wikipedia.org, using TopN to specify the number of search results. It supplements the existing datasets.",
    "yahooFinance": "YahooFinance",
    "yahooFinanceDescription": "A component that queries information about a publicly traded company using its ticker symbol.",
  },
}
