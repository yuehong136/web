...
import { Paperclip, Send, Mic, MicOff, X, Globe } from 'lucide-react'
...
export function ChatInput({
  ...
  const handlePromptSelect = useCallback(
    (prompt: PromptItem) => {
      ...
      const afterSearchText = currentValue.slice(
        atTriggerPosition + 1 + promptSearchQuery.length,
      )
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      const shouldSubmit =
        (submitType === 'enter' && e.key === 'Enter' && !e.shiftKey) ||
    ...
    >
      <div
        className={
          floating
            ? classNames.wrapper
            : `mx-auto max-w-4xl ${classNames.wrapper}`
        }
      >
        {/* 头部内容 */}
        ...
        {attachedFiles.length > 0 && (
          <div className="mb-4 flex flex-wrap gap-2">
            {attachedFiles.map((file, index) => (
              <div
                key={index}
                className="flex items-center gap-2 rounded-lg border border-[var(--color-components-tag-border)] bg-[var(--color-components-tag-bg)] px-3 py-2 text-sm"
              >
                <span className="text-[var(--color-components-tag-text)]">
                  {file.name}
                </span>
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
                  className="h-4 w-4 p-0 text-[var(--color-components-tag-text)] opacity-60 hover:bg-[var(--color-components-tag-bg-hover)] hover:opacity-100"
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            >
              ...
              {/* 右侧操作按钮区 */}
              <div
                className={`flex items-center gap-2 ${classNames.actions}`}
              ...
              >
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="h-8 w-8 p-0 text-[var(--color-text-tertiary)] hover:bg-[var(--color-background-subtle)] hover:text-[var(--color-text-primary)]"
                ...
                {loading ? (
                  <Button
                    ...
                    size="sm"
                    variant="default"
                    className="h-8 bg-[var(--color-components-button-primary-bg-disabled)] px-4 text-[var(--color-components-button-primary-text-disabled)]"
                    disabled
                  >
                    发送中...
                  </Button>
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="default"
                    className="h-8 bg-[var(--color-components-button-primary-bg)] px-4 text-[var(--color-components-button-primary-text)] hover:bg-[var(--color-components-button-primary-bg-hover)] disabled:bg-[var(--color-components-button-primary-bg-disabled)] disabled:text-[var(--color-components-button-primary-text-disabled)]"
