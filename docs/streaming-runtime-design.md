...
8. ~~`useSearchExecution.ts`~~（✅ 2026-06-12 已迁移：仅 SUMMARIZING 段是 SSE，读循环换 `assertSSEResponse` + `readSSEStream<SSEEnvelope>`；**signal 传给了 readSSEStream**——原循环自带 `signal.aborted → reader.cancel() → break`，catch 查 `signal.aborted` 布尔（局部 const 无 ref 竞态），干净 resolve 后由 allSettled 之后的 `signal.aborted` 检查进 STOPPED，终态与原 break 路径一致；rAF 批处理（scheduleStreamPatch/flushPendingStreamPatch）与 6 阶段状态机一行未动，循环尾 finalize+flush 原样保留在 resolve 之后；onEvent 内 `getStreamErrorMessage` 非零码即 throw 的语义不变（throw 传播进 allSettled rejected 分支）；`parseErrorMode` 缺省 'ignore' 对应原显式 JSON.parse catch-continue；检索/相关问题等非 SSE 请求及其 signal 检查点未触碰）
9. ~~`ExplorePage.tsx` 内联两段~~（✅ 2026-06-12 已迁移：app completion 与 `/v1/chat_service_sse` 兜底两段循环均换 `assertSSEResponse` + `readSSEStream<SSEEnvelope>`，onEvent 内 references 合并/消息更新逻辑逐行原样；**signal 传给了 readSSEStream 且补上了 fetch 接线**——迁移前 AbortController 创建后 signal 从未接到 completion/fetch/读循环任何一处，「停止输出」按钮只翻 UI 标志、流继续后台消费（与 useHomeChat app 模式迁移前同型，owner 点名照 46b659c 修复）；controller 改局部捕获防 handleStopOutput 先 abort 后置 null ref 的竞态，catch 开头判局部 `abortController.signal.aborted` 短路——fetch 挂 signal 后用户停止会抛 AbortError，不短路会被误判为发送失败并回滚消息（迁移自身引入回归的必要防护）；`parseErrorMode` 两段均缺省 'ignore' 对应原「JSON 解析错误，忽略」；本期只做流式迁移未拆文件（拆分是独立债务），棘轮基线同步收紧）
