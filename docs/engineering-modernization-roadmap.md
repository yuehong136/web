...
| 2026-06-11 | 迁移 `use-pipeline-workbench.ts`（5/9，实扫补记面）：手写 `split(/\r?\n/)` + `data:` 剥离换 `readSSEStream` + Promise 早返回；拿到 message_id 后流转后台排空、不 cancel 连接（运行由服务端继续，停止另走 cancelDataflow）；本面手写的 !ok/!body 检查换 assertSSEResponse                            | （见 git log） | 不传 signal 给 readSSEStream，保 AbortError → STOPPED；`extractMessageIdFromChunk` util 零改动（吃 rawData）；解析样板 grep 零命中                                                                                                                                                                                                                                     |
