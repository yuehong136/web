...
| 指标                |                                                                  2026-08-13 实测 | 结论                                                                          |
| ------------------- | -------------------------------------------------------------------------------: | ----------------------------------------------------------------------------- |
| `src` 下 `.ts/.tsx` |                                                                          1408 个 | ENG-6 删除 3 个死入口文件并新增能力策略、payload 与回归测试                   |
| 测试文件            |                                                73 个（`src` 72 + ESLint 规则 1） | 正式六个脚本直接纳入 30 个；文件数不等于语句覆盖率                            |
| lint                |                                           0 errors / 1485 warnings（357 个文件） | 527 个 `no-explicit-any`；可访问性警告仍为主要存量                            |
| 文件体积债务        |                                                                    32 个在册文件 | 棘轮通过；ApiKeysPage 已从 4021 收紧至 3923 行，仍需继续拆分                  |
| Bundle 三预算       |           总 JS 25.67/26.13 MB；入口 gzip 115/120 KB；最大 chunk gzip 723/739 KB | 预算使用率均超过 96%，且只覆盖 `dist/js`                                      |
| 测试实跑            | API 92、Agent 72、Product UI 3+10、Streaming 43+2、Design Token 11、Security 2+1 | 本轮通过；没有真实 Playwright E2E                                             |
...
| --: | ------ | ---------------------------------------------------------------------------------------------------- | :----: | --------------: | ------------------------------------------------- | -------- |
|   1 | ENG-7  | 产品化 404、根 `errorElement`/ErrorBoundary、统一 mutation 错误反馈                                  |   P1   |     M，3–5 人日 | 纯前端                                            | 未开始   |
|   2 | ENG-8  | 忘记密码、发布、导出、API Key 测试等正式业务闭环                                                     |   P1   |    L，5–10 人日 | 跨前后端；需接口、权限和状态机契约                | 未开始   |
|   3 | ENG-9  | 修正 Studio 分页 KPI、Memory 硬编码统计等数据口径                                                    |   P1   |   M–L，3–6 人日 | 跨前后端；优先服务端聚合字段                      | 未开始   |
|   4 | ARCH-7 | 统一 SSE 终态、Abort/timeout、REST/SSE 401 与意外 EOF 语义                                           |   P1   |    L，6–10 人日 | 前端主导；最好先约定服务端完成帧                  | 未开始   |
|   5 | SEC-1  | 会话凭证迁移至 HttpOnly/Secure/SameSite refresh cookie 或 BFF                                        |   P1   |     L，5–8 人日 | 后端另约 5–10 人日；含 CSRF/跨标签页设计          | 未开始   |
|   6 | SEC-2  | CSP Report-Only、nonce/hash、违规监控与强制模式                                                      |   P1   |     M，3–5 人日 | 前端/部署；另需约 1 周观察期                      | 未开始   |
|   7 | ENG-10 | 错误、Web Vitals、LLM TTFT、流中断率、取消率和 Trace ID 可观测性                                     |   P1   |     L，5–8 人日 | 平台另约 2–4 人日；先定义隐私边界                 | 未开始   |
|   8 | ENG-2  | 统一 `test:unit`/CI，并增加三条 Playwright 黄金链路                                                  |   P1   | L–XL，9–14 人日 | 需稳定测试账号、数据和接口；依赖 ENG-6～9、ARCH-7 | 部分完成 |
|   9 | HYG-2  | 对 5 个生产依赖告警做可达性验证，分批升级并引入依赖自动化                                            |   P1   |   S–M，2–5 人日 | 若证实浏览器生产路径可利用，对应项升 P0           | 部分完成 |
|  10 | ARCH-2 | auth/Agent run/Knowledge 试点 OpenAPI codegen + Zod；收口绕过 API/Query 边界的旧页面                 |   P2   | L–XL，8–15 人日 | 后端另约 4–8 人日；需稳定 OpenAPI                 | 部分完成 |
|  11 | ENG-11 | Bundle 门禁扩展到真实路由 preload、全部 `dist` 和静态资产，并优化登录图/超大 SVG/Monaco/Ant Design X |   P2   |   M–L，4–7 人日 | 最好先有 ENG-10 RUM 基线                          | 未开始   |
|  12 | ENG-3  | a11y 棘轮、核心模板小屏降级、i18n/route locale 收口、虚拟化和 mutation UX                            |   P2   | XXL，28–45 人日 | 拆成 4–6 个独立 PR；需产品确认移动端范围          | 未开始   |
|  13 | HYG-3  | 修正文档版本表、环境变量登记和路线图状态漂移                                                         |   P2   |   S–M，2–4 人日 | AGENTS.md/CLAUDE.md 双语同提交                    | 未开始   |
|  14 | ARCH-8 | 共享 Chat Workbench：composer、附件、滚动、停止/重试、工具调用、反馈和可访问状态                     |   P2   |  XL，15–25 人日 | 依赖 ARCH-7、ENG-2；需稳定消息/工具协议           | 未开始   |
|  15 | ENG-12 | 按活跃度拆 ApiKeys、Provider modal、MCPChat、Explore，并收紧文件棘轮                                 |   P2   | XXL，25–40 人日 | 先修业务正确性，避免重构错误行为                  | 未开始   |
|  16 | ARCH-9 | 试点 Projects/Spaces + Assets 信息架构，整合聊天、文件、知识、指令与 Studio                          |   P3   | XXL，25–45 人日 | 跨产品/前后端；需数据模型、权限和迁移方案         | 未开始   |
|  17 | ARCH-5 | Tailwind 4 评估与设计令牌内部简化                                                                    |   P3   |    L，8–12 人日 | 可靠性、体验和 Bundle 工作稳定后再做              | 未开始   |

### 推荐执行波次

1. **信任与恢复（立即接续）**：#1，约 3–5 前端人日；ENG-6 已完成，下一步统一路由错误恢复。
2. **核心可靠性（未来 2～4 周）**：#2～#9，约 38–66 前端人日，建议 2～3 人并行；跨后端事项先冻结契约再实施。
3. **工程规模化（1～2 个季度）**：#10～#15，按模块持续偿还，不做全仓机械迁移。
4. **产品架构试点**：#16 先做单一 Project/Space，不直接重构全部导航。
5. **工具链尾项**：#17 不得排在安全、运行闭环、E2E 和观测之前。
...
| 2026-08-13 | 实现与验收 | 本次提交 | 真实接通 Agent 设置；其余入口隐藏/禁用；新增 7 个测试文件和正式 `test:product-ui` CI 门禁 |
...
| 1：信任与恢复 | ENG-7                                      | ENG-6 capability 清单已冻结  | 所有路由错误可恢复；mutation 失败有一致反馈                                |
