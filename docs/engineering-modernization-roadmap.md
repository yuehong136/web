...
| 2026-06-12 | knowledge-logs 域：新建 `knowledgeLogKeys`（pages/knowledge/logs/constants.ts），`list(tab,…)`（LogTabType 枚举值即原 'fileLogs'/'datasetLogs' 前缀）与 `stats(kbId)` 形状均不变；use-log-list-state / use-log-stats 迁移                                                                                                                                                        | （待填） | 形状无变化；两 key 均无失效方/写入方配对（refetch 手动）          |
