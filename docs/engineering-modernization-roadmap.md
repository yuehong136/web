...
| 2026-06-12 | 删除死文件 `use-auth.ts`（196 行）+ `use-conversations.ts`（299 行）——中央 user/conversations 两域的唯一消费方，连带消灭内联 `['auth','channels']`、`['conversation','stats',timeRange]` | （待填） | 删除前复扫全部导出零导入；行为零漂移（无人引用）                  |
