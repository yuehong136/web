...
| 2026-06-12 | documents 页面 key：`documentKeys` 新增 `standaloneDetail(docId)`（=原 `['documentDetail',…]`）与 `chunkList(...)`（=原 `['documentChunks',…]`），均保独立根——刻意不并入 `['documents']` 前缀，避免 metadata 域对该前缀的失效新增命中；KnowledgeChunksLayout / use-chunk-list-state 迁移                                                                                         | （待填） | 形状无变化；两 key 均无失效方/写入方配对                          |
