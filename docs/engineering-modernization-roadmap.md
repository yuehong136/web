...
> 最新复核：2026-08-13，基准提交：`01cc261`；新增当前排序、工作量估算与运行态证据，原始基线保留用于比较。
...
- **P0**：源码或运行态已高置信确认的账号接管、跨租户越权、凭证泄漏或注入路径，立即止血；静态确认必须注明未做动态利用，未验证候选不直接记 P0。
...
| 指标                |                                                                  2026-08-13 实测 | 结论                                                                          |
| ------------------- | -------------------------------------------------------------------------------: | ----------------------------------------------------------------------------- |
| `src` 下 `.ts/.tsx` |                                                                          1422 个 | 按本提交范围计数；不含并行未提交的 ARCH-7 runtime helper/test                 |
| 测试文件            |                                                76 个（`src` 75 + ESLint 规则 1） | 正式六个脚本直接纳入 33 个；文件数不等于语句覆盖率                            |
...
| 测试实跑            | API 99、Agent 72、Product UI 3+30、Streaming 43+2、Design Token 11、Security 2+1 | 最近实跑基线；本切片 Product UI 3+30 通过；没有真实 Playwright E2E            |
...
5. **产品架构试点**：#17 先做单一 Project/Space，不直接重构全部导航。
6. **工具链尾项**：#18 不得排在安全、运行闭环、E2E 和观测之前。
...
- **状态**：未开始（2026-08-13 仅静态候选；验证成立立即升 P0）
- **优先级 / 工作量**：P1 / 授权验证 XS，0.5–1 人日；若成立，前端 S，1–2 人日，后端 M，3–5 人日
- **问题与证据**：MultiRAG `/chatbots/{dialog_id}/info|completions` 的 beta-token dependency 已返回 `tenant_id`，但两个 handler 以 `_` 丢弃并按 path `dialog_id` 直接读取/运行 Dialog；`async_iframe_completion` 也只按 ID 取 Dialog/会话。deprecated `/conversation/set` 的更新路径与 `/conversation/completion` 的读取/执行路径未在 handler 中看到 conversation/dialog owner 约束。下游仍可能存在尚未识别的拦截，因此当前只登记跨租户授权候选，不声称 IDOR 已成立。
- **立即约束**：Studio 发布/分享入口继续隐藏；不得用真实租户或用户数据验证。验证只能创建两个获授权 disposable tenant、各自的 Dialog/Conversation 和临时 token，并保存 allow/deny 证据后立即清理。
- **方案**：所有 chatbot 查询和执行把鉴权返回的 tenant 作为数据库谓词的一部分；session 必须同时绑定 `dialog_id + tenant_id`。deprecated conversation 路由要么删除，要么统一迁到 Principal/owner policy，更新前后都校验 user/tenant ownership，禁止通过更新请求改写 owner。
- **验收**：同租户合法调用通过，跨租户 dialog/session 全部返回一致的 404/403 且不泄露资源存在性；直接 ID、已有 session、stream/non-stream、info/completion 四类矩阵均覆盖；静态查询 helper 强制接收 tenant/user scope。
- **状态与进展记录**：

| 日期       | 动作         | 提交 | 备注                                                                                             |
| ---------- | ------------ | ---- | ------------------------------------------------------------------------------------------------ |
| 2026-08-13 | 静态候选立项 | —    | 已定位 source→authorization gap；尚未完成授权的 disposable 环境验证，保持 P1，不夸大为已证实漏洞 |
...
- **状态**：F0 文档基线与 CLP-DESK0 非发布态安全壳切片已完成（2026-08-13）；Web 仍是唯一生产产品，CLP-DESK/MVP 未完成
...
| 2026-08-13 | DESK0 安全壳基线落地      | 当前实现提交 | Electron main/sandbox preload、`app://bundle/`、独立 Rolldown、严格 staging、ASAR/fuses 与 macOS arm64 unpacked smoke；不含认证、Run/updater/Host、Windows 或正式签名发布 |
...
| Search 导出 `/search/:id`                       | 已开放本地 Markdown 导出     | 从当前可见会话按字段白名单导出；流式期间禁用，排除 thinking、错误详情与内部引用 ID/chunks；服务端导出仍待 ENG-8 后续合同                                                                  |
...
| 2026-08-13 | 实现与验收 | `03915ae` | 根恢复边界、产品 404、安全 mutation 反馈、深链恢复、i18n 与正式测试门禁完成；观测转 ENG-10 |

### ENG-8 用户可见业务闭环

- **状态**：部分完成（2026-08-13，Search 本地导出第一切片）
- **优先级 / 剩余工作量**：P1 / 前端 M–L，4–8 人日；后端 XL，13–25 人日（不含 SEC-7/SEC-8）
- **依赖**：Studio publication/version、API runner/用例/环境、API Key 轮换等接口及权限/状态机契约；密码重置已拆到 SEC-7
- **问题**：ENG-6 已停止目标入口误导，但不能代替正式业务交付。Search 当前可见会话的本地导出已完成；Studio 发布、API 在线调试/用例/环境保存与 API Key 安全轮换仍缺接口和状态机。API Key “重新生成”当前为 DELETE 后 POST，第二步失败会永久丢失旧 key。Documents/Workflow/Knowledge Import 等占位面也仍需逐一确认正式导航策略。
- **方案**：按使用价值逐项确定“实现 / 保持 disabled / 移出正式导航”；实现项必须定义请求、幂等、权限、进行中、成功、失败、重试、审计和回滚状态；有副作用的发布/工具调用补确认与结果可追踪性。Search 第一切片不等待不存在的服务端导出 API：只把浏览器当前可见会话按字段白名单导出为 Markdown，默认排除 thinking、错误详情、tenant/KB/document/chunk ID 与原始 chunks，流式期间禁用，并对动态 Markdown 文本、文件名和 object URL 生命周期设安全边界。
- **验收**：每个保留的主操作都有真实接口和状态机，不以延时或随机数模拟；黄金 E2E 覆盖 Studio 保存→预览→发布，其他闭环至少有契约测试。

#### 2026-08-13 后端合同核验

- **Studio 发布**：当前保存直接覆盖同一可变 Dialog，没有 immutable publication snapshot、draft revision、publication pointer、expected revision/CAS、idempotency、rollback 或 app-scoped share grant；现有 `status` 还是有效/软删除字段，不是发布状态。目标合同应引入不可变 `DialogPublication`、草稿 revision、幂等发布与指针式回滚，以及仅存摘要、可撤销、绑定 app/publication 的分享 token。在此之前发布/分享入口继续隐藏。
- **API 在线调试 / 保存用例**：当前没有通用 runner 或 API 请求用例接口；Evaluation case 是 RAG 评测数据集，不是 HTTP 请求用例，不能冒充复用。浏览器直接请求受 CORS 限制且会把前端暴露在任意目标与凭证外发风险下；服务端代理必须先完成 http(s) 限制、目标 allowlist/公网 IP 校验、DNS pinning、逐跳重定向复检、方法/端口/请求响应体预算、超时、并发/速率限制、响应头清洗和无敏感日志审计，再开放入口。
- **环境与密钥**：环境 CRUD 已存在，但 `is_secret` 变量仍以明文存储并在详情/resolve 响应返回；`base_url` 目前也未达到 runner 的 SSRF 写入边界。正式 runner 不得把当前 Web JWT 自动转发到目标站点，secret 应在服务端运行时解析且响应只回掩码元数据。
- **API Key 轮换**：后端只有 GET/POST/DELETE，当前前端“重新生成”先 DELETE 再 POST，第二步失败会永久中断；列表还会重复返回完整 token/beta，资源也没有稳定 opaque key ID。后端应先提供重叠轮换状态机（create new → 用户迁移/验证 → disable old → grace period 后 revoke），secret 仅创建时展示一次；在此之前不得把 DELETE+POST 描述为安全轮换。
- **状态与进展记录**：

| 日期       | 动作            | 提交      | 备注                                                                                                                          |
| ---------- | --------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-13 | 复核立项        | —         | 确认跨端合同与阻塞；Search 选择本地字段白名单导出作为独立切片，其余入口保持 disabled/隐藏                                     |
| 2026-08-13 | Search 导出切片 | `01cc261` | 从当前内存态 turns 生成安全 Markdown；恢复可访问入口，流式禁用；focused serializer/download/header 测试进入 `test:product-ui` |

### ENG-9 统计与筛选数据口径正确性

- **状态**：未开始
- **优先级 / 工作量**：P1 / M–L，3–6 前端人日；后端聚合字段另估
- **问题与证据**：Studio 不仅把当前分页 `dialogApps` 的发布/草稿/最近更新 KPI、筛选和排序与服务端全集 total 同屏，还把 Dialog `status=1/0` 解释成“已发布/草稿”；后端模型和 `DialogService` 明确把该字段用于 VALID/INVALID（有效/软删除），列表只返回 VALID，因此当前可见项会被系统性误标为“已发布”。Memory 还使用 `1280`、`45.6 MB`、`total * 0.8` 等硬编码/推导统计。企业租户数据量增大时会同时出现错误状态、错误 KPI 和“总数很多但筛选为空”。
- **方案**：立即停止把 `status` 映射成 publication 状态；在 ENG-8 的真实 publication 合同上线前移除发布/草稿 KPI 与筛选，或明确标为当前页/有效资源。服务端再返回全量聚合和排序/筛选结果；前端只展示有来源、口径和更新时间的数据。暂时拿不到真实字段时删除卡片，不能猜算全局值。
