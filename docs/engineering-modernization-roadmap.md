...
| 2026-06-12 | explore 域：`chatKeys` 新增 `dialogConversations(dialogId)`（=原 ExplorePage 内联 `['dialogConversations',…]`，形状不变）；与 `conversationsByDialog` 同 API 但查询配置不同，刻意不共 key（共 key 属未来可选归并，记备注）                                                                                                                                                       | （待填） | 形状无变化；无失效方配对（refetchConversations 手动）             |
