...
| 2026-06-22 | 修复 44c3b14（dialog 迁移）引入的棘轮回归：新文件 use-create-app-page.ts 682>600、types/api.ts 1604→1606 | 待回填                         | ARCH-4 开工前发现 master 门禁已红。拆 `handleSave`→`use-create-app-save.ts`（682→557）；删未使用的 `DialogTemplateFile`（types/api.ts 1606→1599，基线收紧 1604→1599）。零行为变更，门禁回绿 |
