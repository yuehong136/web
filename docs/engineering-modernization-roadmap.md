...
| 2026-06-12 | metadata 域：新建 `metadataKeys`（use-metadata.ts，all/summary 逐元素复刻中央，含 `docIds?.join(',') ?? 'all'` 尾段）；跨域 `invalidateQueries.documents()`×3 → `documentKeys.all`（`['documents']` 逐元素相等，metadata 变更继续命中文档列表）；useUpdateDocumentMetadataSettings 补 `useQueryClient()`；行 193 的 `queryKeys.knowledgeBases.detail` 死失效留待下一 fix 提交                                      | （待填） | 形状全部无变化；跨域 documents 失效经实证根相等                              |
