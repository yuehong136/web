...
| 2026-06-12 | knowledge 域：`knowledgeKeys` 新增 `graph(id)`（收编 use-knowledge-request:186 的 `[...detail(id),'graph']` 展开，形状不变）与 `simpleList()`（=原 use-chat-settings 内联 `['knowledgeBases']`，缓存为 kbs 数组与 `list()` 分页形态不同，刻意不共 key）                                                                                                                          | （待填） | 形状均无变化；`['knowledgeBases']` 无失效方（中央同名 helper 无人调用） |
